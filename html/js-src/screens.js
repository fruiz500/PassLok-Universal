	...
	fileReader.onload = function(fileLoadedEvent){
		...
		if(URLFromFileLoaded.slice(0,10) == "data:image"){						//if it's an image, trigger image decryption
			previewImg.onload = function(){
				...
				if(filteredEmail == "Who knows?") filteredEmail = '';
				if(filteredEmail && soleRecipient){									//don't do it unless I am the only recipient
					...
					stegoImageMsg.textContent =  'The image Password has been pre-filled as if you were the only recipient. Edit if not and click Decrypt';
				}else{
					stegoImageBox.value = '';
